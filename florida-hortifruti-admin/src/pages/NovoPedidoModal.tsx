...
export function NovoPedidoModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [itens, setItens] = useState<{ produtoId: string; nome: string; quantidade: number; valorUnitario: number }[]>([])
  ...
  function reset() {
    setClienteId(''); setItens([]); setValorFrete(''); setDescontoValor('')
    setFormaPagamento('PIX'); setDataVencimento(''); setNecessitaNF(false); setObservacoes('')
  }

  function add(p: any) {
    const existe = itens.find((i) => i.produtoId === p.id)
    if (existe) setItens(itens.map((i) => i.produtoId === p.id ? { ...i, quantidade: i.quantidade + 1 } : i))
    else setItens([...itens, { produtoId: p.id, nome: p.nome, quantidade: 1, valorUnitario: Number(p.precoSugerido) }])
  }
  ...
  function dec(id: string) {
    ...
    else setItens(itens.map((i) => i.produtoId === id ? { ...i, quantidade: i.quantidade - 1 } : i))
  }
  ...
  return (
    <Modal open={open} onClose={() => { reset(); onClose() }} title="Novo Pedido" size="lg">
      <form onSubmit={(e) => { e.preventDefault(); criar.mutate() }} className="space-y-4">
        <Select label="Cliente *" value={clienteId} onChange={(e) => setClienteId(e.target.value)} required>
          ...
          {clientes.map((c: any) => <option key={c.id} value={c.id}>{c.razaoSocialOuNome}</option>)}
        </Select>
              ...
              return (
                <div key={p.id} className="flex items-center justify-between gap-2 text-sm">
                  ...
                  <div className="flex items-center gap-1">
                    {item && (
                      <>
                        <button type="button" onClick={() => dec(p.id)} className="w-6 h-6 bg-red-100 text-red-600 rounded flex items-center justify-center"><Minus className="w-3 h-3" /></button>
                        <span className="w-6 text-center font-semibold">{item.quantidade}</span>
                      </>
                    )}
                    <button type="button" onClick={() => add(p)} className="w-6 h-6 bg-green-100 text-green-700 rounded flex items-center justify-center"><Plus className="w-3 h-3" /></button>
                  </div>
                </div>
              )
            })}
        ...
        <div className="grid grid-cols-2 gap-3">
          <Input label="Frete (R$)" type="number" step="0.01" value={valorFrete} onChange={(e) => setValorFrete(e.target.value)} />
          <Input label="Desconto (R$)" type="number" step="0.01" value={descontoValor} onChange={(e) => setDescontoValor(e.target.value)} />
          <Select label="Pagamento *" value={formaPagamento} onChange={(e) => setFormaPagamento(e.target.value)}>
        ...
        <label className="flex items-center gap-2 text-sm text-gray-700">
          <input type="checkbox" checked={necessitaNF} onChange={(e) => setNecessitaNF(e.target.checked)} className="accent-green-600" />
