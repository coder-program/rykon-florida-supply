...
const STATUS_ACOES: Record<string, { label: string; next: string }[]> = {
  ...
  EM_CONFERENCIA:    [{ label: 'Aprovar', next: 'aprovar' }, { label: 'Cancelar', next: 'cancelar' }],
  APROVADO:          [{ label: 'Em separação', next: 'separacao' }, { label: 'Cancelar', next: 'cancelar' }],
...
export function PedidosPage() {
  ...
  const { data: pedidos = [], isLoading } = useQuery({
    ...
    queryFn: () => api.get('/pedidos', { params: filtroStatus ? { status: filtroStatus } : {} }).then((r) => r.data),
  })

  const mudarStatus = useMutation({
    mutationFn: ({ id, acao }: { id: string; acao: string }) => api.post(`/pedidos/${id}/${acao}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['pedidos'] }); setPedidoSelecionado(null) },
  })

  const editarPedido = useMutation({
    mutationFn: (d: any) => api.patch(`/pedidos/${pedidoSelecionado.id}`, d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['pedidos'] }); setEditando(false); setPedidoSelecionado(null) },
  })
  ...
  const pedidosFiltrados = pedidos.filter((p: any) =>
    !busca || p.numero?.toString().includes(busca) ||
  ...
  return (
    <div>
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <div className="w-48">
            <Select value={filtroStatus} onChange={(e) => setFiltroStatus(e.target.value)}>
              ...
              {Object.entries(STATUS_PEDIDO_LABEL).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
            </Select>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {pedidosFiltrados.map((p: any) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <p className="text-xs text-gray-600">{FORMA_PAGAMENTO_LABEL[p.formaPagamento]}</p>
                    <Badge className={`mt-0.5 ${STATUS_PAGAMENTO_COLOR[p.statusPagamento]}`}>{p.statusPagamento?.replace('_', ' ')}</Badge>
                  </td>
                  <td className="px-4 py-3 text-right font-semibold text-gray-900">{formatBRL(p.totalFinal)}</td>
                  <td className="px-4 py-3">
      ...
      <Modal open={!!pedidoSelecionado} onClose={() => setPedidoSelecionado(null)} title={`Pedido #${String(pedidoSelecionado?.numero ?? '').padStart(6, '0')}`} size="lg">
        {pedidoSelecionado && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-gray-500">Cliente:</span> <strong>{pedidoSelecionado.cliente?.razaoSocialOuNome}</strong></div>
              <div><span className="text-gray-500">Vendedor:</span> <strong>{pedidoSelecionado.vendedor?.nome}</strong></div>
              <div><span className="text-gray-500">Data:</span> {formatDate(pedidoSelecionado.data)}</div>
              <div><span className="text-gray-500">Pagamento:</span> {FORMA_PAGAMENTO_LABEL[pedidoSelecionado.formaPagamento]}</div>
              {pedidoSelecionado.observacoes && <div className="col-span-2"><span className="text-gray-500">Obs:</span> {pedidoSelecionado.observacoes}</div>}
            </div>
            ...
            <table className="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="text-right px-3 py-2 text-xs font-medium text-gray-500">Unitário</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {pedidoSelecionado.itens?.map((i: any) => (
                  <tr key={i.id}>
                    ...
                    <td className="px-3 py-2 text-right font-semibold">{formatBRL(i.valorTotal)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="bg-gray-50 text-sm">
                <tr><td colSpan={3} className="px-3 py-1.5 text-right text-gray-500">Subtotal</td><td className="px-3 py-1.5 text-right">{formatBRL(pedidoSelecionado.subtotal)}</td></tr>
                {Number(pedidoSelecionado.valorFrete) > 0 && <tr><td colSpan={3} className="px-3 py-1 text-right text-gray-500">Frete</td><td className="px-3 py-1 text-right">+{formatBRL(pedidoSelecionado.valorFrete)}</td></tr>}
                {Number(pedidoSelecionado.descontoValor) > 0 && <tr><td colSpan={3} className="px-3 py-1 text-right text-gray-500">Desconto</td><td className="px-3 py-1 text-right text-red-600">-{formatBRL(pedidoSelecionado.descontoValor)}</td></tr>}
                <tr className="font-bold"><td colSpan={3} className="px-3 py-2 text-right">Total</td><td className="px-3 py-2 text-right text-green-700">{formatBRL(pedidoSelecionado.totalFinal)}</td></tr>
              </tfoot>
            </table>

            {/* Edição antes de aprovar */}
            {['ENVIADO', 'EM_CONFERENCIA', 'RASCUNHO'].includes(pedidoSelecionado.status) && !editando && (
            ...
            {editando && (
              <form
                onSubmit={(e) => { e.preventDefault(); editarPedido.mutate({ ...formEdit, valorFrete: Number(formEdit.valorFrete), descontoValor: Number(formEdit.descontoValor) }) }}
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" step="0.01" value={formEdit.valorFrete} onChange={(e) => setFormEdit((f) => ({ ...f, valorFrete: e.target.value }))}
                      className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">Desconto (R$)</label>
                    <input type="number" step="0.01" value={formEdit.descontoValor} onChange={(e) => setFormEdit((f) => ({ ...f, descontoValor: e.target.value }))}
                      className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select value={formEdit.formaPagamento} onChange={(e) => setFormEdit((f) => ({ ...f, formaPagamento: e.target.value }))}
                      className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500">
                      {['PIX', 'BOLETO', 'DINHEIRO', 'OUTROS'].map((v) => <option key={v} value={v}>{v}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">Vencimento</label>
                    <input type="date" value={formEdit.dataVencimento} onChange={(e) => setFormEdit((f) => ({ ...f, dataVencimento: e.target.value }))}
                      className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Observações</label>
                  <textarea rows={2} value={formEdit.observacoes} onChange={(e) => setFormEdit((f) => ({ ...f, observacoes: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 resize-none" />
                </div>
                <label className="flex items-center gap-2 text-sm text-gray-600">
                  <input type="checkbox" checked={formEdit.necessitaNF} onChange={(e) => setFormEdit((f) => ({ ...f, necessitaNF: e.target.checked }))} className="accent-green-600" />
                  Necessita Nota Fiscal
                </label>
                <div className="flex gap-2">
                  <Button type="button" variant="secondary" onClick={() => setEditando(false)}>Cancelar</Button>
                  <Button type="submit" disabled={editarPedido.isPending}>{editarPedido.isPending ? 'Salvando...' : 'Salvar Alterações'}</Button>
                </div>
              </form>
            )}
              ...
              <div className="flex gap-2 flex-wrap pt-2 border-t border-gray-100">
                ...
                {pedidoSelecionado.etiqueta && (
                  <>
                    ...
                    <Button
                      variant="ghost"
                      onClick={() => window.open(`/api/p/${pedidoSelecionado.etiqueta.tokenPublico}`, '_blank')}
                    >
