...
function getPeriodoDates(p: string) {
  ...
  if (p === '7d') {
    ...
    i.setDate(hoje.getDate() - 7)
    return { dataInicio: fmt(i), dataFim: fmt(hoje) }
  }
  if (p === '30d') {
    const i = new Date(hoje)
    i.setDate(hoje.getDate() - 30)
    return { dataInicio: fmt(i), dataFim: fmt(hoje) }
  }
  // mes
...
export function FinanceiroPage() {
  ...
  const { data: contas = [], isLoading: loadingContas } = useQuery({
    ...
    queryFn: () =>
      api
        .get('/financeiro/contas-a-receber', {
          params: {
    ...
    <div>
      ...
      <div className="space-y-6 p-4 md:p-6">
        ...
        <div className="space-y-3 md:hidden">
          {loadingContas && <p className="py-8 text-center text-sm text-gray-400">Carregando...</p>}
          {!loadingContas && contas.length === 0 && (
            <p className="py-8 text-center text-sm text-gray-400">Nenhuma conta encontrada</p>
          )}
          {contas.map((c: any) => {
            ...
            return (
              <div
                key={c.id}
                className={`rounded-xl border border-gray-200 bg-white p-4 ${sit.row}`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-semibold text-gray-900">{c.cliente?.razaoSocialOuNome}</p>
                    <p className="text-xs text-gray-400">
                      #{String(c.numero).padStart(6, '0')} · {formatDate(c.data)}
                    </p>
                  </div>
                  <p className="text-sm font-semibold">{formatBRL(c.totalFinal)}</p>
                </div>
                ...
                <div className="mt-3">
                  {c.situacaoCalculada !== 'PAGO' ? (
                    ...
                    <Button
                      ...
                      variant="secondary"
                      onClick={() => {
                        if (confirm('Reabrir esta conta como em aberto?')) reabrir.mutate(c.id)
                      }}
                          ...
                          <div className="flex items-center gap-1 justify-end">
                            {c.situacaoCalculada !== 'PAGO' ? (
                              ...
                              <Button
                                ...
                                variant="secondary"
                                onClick={() => {
                                  if (confirm('Reabrir esta conta como em aberto?'))
                                    reabrir.mutate(c.id)
                                }}
                                disabled={reabrir.isPending}
                              >
                          ...
                          <td colSpan={8} className="px-6 pb-4 pt-1">
                            <div className="text-xs text-gray-500 space-y-1 bg-white/70 rounded-lg p-3 border border-gray-100">
                              <div className="grid grid-cols-3 gap-4">
                                ...
                                {Number(c.valorFrete) > 0 && (
                                  <div>
                                    <span className="text-gray-400">Frete:</span>{' '}
                                    {formatBRL(c.valorFrete)}
                                  </div>
                                )}
                                {Number(c.descontoValor) > 0 && (
                                  <div>
                                    <span className="text-gray-400">Desconto:</span> -
                                    {formatBRL(c.descontoValor)}
                                  </div>
                                )}
                                <div>
                                  <span className="text-gray-400">NF:</span>{' '}
                                  {c.necessitaNF ? '✅ Sim' : 'Não'}
                                </div>
                                {c.condicaoNegociada && (
                                  <div>
                                ...
                                {c.cliente?.telefone && (
                                  <div>
                                    <span className="text-gray-400">Tel cliente:</span>{' '}
                                    {c.cliente.telefone}
                                  </div>
                                )}
                              </div>
                              {c.observacoes && (
                                <p className="mt-2 text-gray-600 italic">"{c.observacoes}"</p>
                              )}
                            </div>
                          </td>
                        </tr>
              ...
              <div className="flex flex-col gap-1 sm:flex-row sm:gap-6">
                <span className="text-gray-500">
                  ...
                  <strong className="text-gray-900">
                    {formatBRL(
                      contas
                        .filter((c: any) => c.situacaoCalculada !== 'PAGO')
                        .reduce((s: number, c: any) => s + Number(c.totalFinal), 0),
                    )}
                  </strong>
                </span>
                <span className="text-gray-500">
